...
def main():
    ...
    while x < number_to_create:
        x += 1
        create_package()
    print 'Finished creating ' + str(x) + ' packages.\n'
...
def verify_jss_details():
    ...
    number_to_create = int(prompt_user('Enter number of packages to create: '))


def create_package():
    package_name = str(random.randint(9999, 999999))
    package_string = '<package><name>package' + package_name + '</name><filename>' + package_name + '.dmg</filename></package>'
    connect_jss('/JSSResource/packages', 'POST', package_string)
