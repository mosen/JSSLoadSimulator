...
def main():
    ...
    while x < number_to_create:
        x += 1
        create_building()
    print 'Finished creating ' + str(x) + ' buildings.\n'
...
def verify_jss_details():
    ...
    number_to_create = int(prompt_user('Enter number of buildings to create: '))


def create_building():
    name_file = open(str(os.getcwd()) + '/resources/US_Cities.txt')
    name_list = name_file.readlines()
    name_file.close()
    building_name = name_list[random.randint(0, len(name_list) - 1)].strip()
    building_string = '<building><name>' + building_name + '</name></building>'
    connect_jss('/JSSResource/buildings', 'POST', building_string)
