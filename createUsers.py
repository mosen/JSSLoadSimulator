...
def main():
    ...
    while x < number_to_create:
        x += 1
        create_user()
    print 'Finished creating ' + str(x) + ' users.\n'
...
def verify_jss_details():
    ...
    number_to_create = int(prompt_user('Enter number of users to create: '))
...
def create_user():
    ...
    name_file.close()
    user_name = name_list[random.randint(0, len(name_list) - 1)].strip()
    user_string = '<user><name>' + user_name + str(random.randint(999, 9999)) + '</name></user>'
    connect_jss('/JSSResource/users', 'POST', user_string)
